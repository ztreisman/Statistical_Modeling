...
```
Explain how this plot illustrates that the coefficient of `gopher` in `lilymod1` is negative?
...
(@) In this lab so far, we have not considered the other variables `seedlings`, `moisture` and `rockiness`, or any interactions between the predictors. Experiment with adding terms and come up with your preferred model. Describe your model. Support your preference by comparing it to other models that you considered. If you want to use the model selection and averaging routines introduced in Jacob Goheen's teaching demo, this would be a reasonable place to try that out. 
