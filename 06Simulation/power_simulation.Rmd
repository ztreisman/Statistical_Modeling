...
Power calculations are an important part of study design, but power calculations based on all but relatively simple statistical models can be horribly complicated or even impossible. Simulation allows one to avoid these complicated or impossible calculations with a bit of programming.
...
For a first example, we'll look at a $t$-test. An key step in setting up a power calculation is determining effect size. (Power calculations can also be used to determine effect size if all other parameters are known, but this is less frequently done.) The effect size for a $t$-test used by `pwr.t.test` is the difference in means divided by the pooled standard deviation, known as Cohen's $d$, after @cohen.

$$
d=\frac{\mu_a-\mu_b}{\sigma_{pooled}}, \quad \sigma_{pooled}=\sqrt{\frac{(n_a-1)\sigma_a^2+(n_b-1)\sigma_b^2}{(n_a-1)+(n_b-1)}}
$$
...
Compute the required sample size for a $t$-test with an effect size of 1 and a desired power of 80%.
...
To see how to use simulation to compute power, we can simulate to confirm this result. 
```{r}
d <- 1 #effect size
...
(@) The `pwr.f2.test` function can be used to compute the power for a linear model. Parameters are $u$, the number of predictors and $v$, the sample size less the number of coefficients, including the intercept. So a model with formula `y~x1+x2` built with 100 data points would have $u=2$ and $v=97$. In general, to find sample size compute $u+v+1$ and round up to the next whole number. The measure of effect size is `f2` and it is the ratio of variance explained by predictors to variance not explained by predictors:
$$
f^2=\frac{R^2}{1-R^2}
$$
Find the sample size needed to detect a linear effect with an $R^2$ of 0.5 with a power of 80%, in the scenario with two predictors: $\hat y=\beta_0+\beta_1x_1+\beta_2x_2$.
...
```
Effect size can be parameterized by the coefficient of $x$ in this model. Determine the sample size needed for 80% power if $\beta_1=1$.
