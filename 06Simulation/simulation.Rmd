...
Question: Given a statistical model, how do we evaluate it? 

model $+$ computer $\implies$ generate simulated data. 
...
Count the number of plants other the plant being considered  within a distance of 2. Call it $N_2(x)$. $CI(x)=3N_2(x)+1$ is the crowding index.
...
So this data point would be a somewhat unlikely output from the model. 
...
ggplot(poly, aes(x,y, fill=id, group=id)) +
  ...
  geom_line(data=df1, aes(x,y, color="H0", group=NULL, fill=NULL), size=1.5, show_legend=F) + 
