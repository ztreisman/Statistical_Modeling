...
# Work with the resume data. What does it take to get a callback? 
resume$job_ad_id <- NULL
resume <- resume %>%
  transmute_all(factor)
summary(resume)

# Build a tree to decide
tree.resume <- tree(received_callback~.-firstname, resume)
summary(tree.resume)
plot(tree.resume)
text(tree.resume,pretty=2) # pretty = how much to abbreviate. See ShelveLoc. 
